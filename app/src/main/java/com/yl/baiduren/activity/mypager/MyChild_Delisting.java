...
import com.yl.baiduren.adapter.MyChild_Delisting_Adapter;
import com.yl.baiduren.adapter.MyChild_Deliting_Drawlay_Adapter;
...
public class MyChild_Delisting extends BaseActivity implements View.OnClickListener, IDeleteCall, BunessListener {
    ...
    private MyChild_Delisting_Adapter adapter;
    ...
    private MyChild_Deliting_Drawlay_Adapter myChildDelitingDrawlayAdapter;
    private Long childUserId = 0l;//子账号id
    ...
    public void initViews() {
        ...
        if (childUserId != 0) {
            tv_zp_details_name.setText(childUserName + "的摘牌");
        }
        myChildDelitingDrawlayAdapter = new MyChild_Deliting_Drawlay_Adapter(MyChild_Delisting.this);
        adapter = new MyChild_Delisting_Adapter(MyChild_Delisting.this);
    ...
    private void query_Deliting(Sussful_Exmple_Body body) {//我的摘牌列表接口
        status=NORMAL;
        body.setPageNo(pn);
        body.setPageSize(100);
        if(childUserId!=0l){
            body.setChildUserId(childUserId);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.et_need:
                ...
                myChildDelitingDrawlayAdapter.setIndex(1);
                break;
            case R.id.et_assert:
                //侧拉债事人资产选择
                delisting_drawlayout.setVisibility(View.VISIBLE);
                myChildDelitingDrawlayAdapter.setIndex(2);
                break;
            ...
            case R.id.qing_que://精确
                Intent intent=new Intent(this, My_Delisting_Query_Serch.class);
                intent.putExtra("childUserId",childUserId);
    ...
    private void queryAmout() {//金额查询
        if (debt_type_adapter != null && debt_type_adapter.getList() != null) {
            Sussful_Exmple_Body type = new Sussful_Exmple_Body(DataWarehouse.getBaseRequest(this));
            if (debt_type_adapter.getSelectedItem().size() != 0) {
                type.settIds(debt_type_adapter.getSelectedItem());
                query_Deliting(type);
            }
        }
    ...
    private void initViewData(List<Debt_Type_Result.CategoryDO> list) {
        myChildDelitingDrawlayAdapter.setList(list);
        delisting_drawlayout.setAdapter(myChildDelitingDrawlayAdapter);
    }
