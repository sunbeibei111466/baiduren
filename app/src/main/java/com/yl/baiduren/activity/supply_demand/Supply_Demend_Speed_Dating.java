...
import com.yl.baiduren.fragment.my_supply_demend_match_speed.My_Demend_Match_Speed_Fragment;
import com.yl.baiduren.fragment.my_supply_demend_match_speed.My_Supply_Match_Speed_Fragment;
...
public class Supply_Demend_Speed_Dating extends BaseActivity implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private ImageView supply_demend_serch;
    private Long match_id;
    private int type=0;

    @Override
    public int loadWindowLayout() {
        return R.layout.supply_demend_speed_dating;
    }

    @Override
    public void initViews() {
        EventBus.getDefault().register(this);
        ImageView supply_demend_back = findViewById(R.id.match_back);
        supply_demend_back.setOnClickListener(this);
        Button mine_add = findViewById(R.id.speed_dating_match);
    ...
    private void initview() {
        ...
        My_Supply_Match_Speed_Fragment supply_hall_fragment = new My_Supply_Match_Speed_Fragment();
        My_Demend_Match_Speed_Fragment demend_hall_fragment = new My_Demend_Match_Speed_Fragment();
        fragments.add(supply_hall_fragment);//供应端
        fragments.add(demend_hall_fragment);//需求端
        // 实例化适配器
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.speed_dating_match://匹配
                if (type==0){
                if (match_id!=null){//匹配供应
                    Intent intent =new Intent();
                    intent.setClass(this,Speed_Supply_Dating_Result.class);
                    intent.putExtra("match_supply_id",match_id);
                    startActivity(intent);
                }else{
                    ToastUtil.showShort(this,"请选择列表供应");
                }

                }else if (type==1){
                    if (match_id!=null){//匹配需求
                        Intent intent =new Intent();
                        intent.setClass(this,Speed_Demd_Dating_Result.class);
                        intent.putExtra("match_demend_id",match_id);
                        startActivity(intent);
                    }else{
                        ToastUtil.showShort(this,"请选择列表需求");
                    }
    ...
    public void getLong_Id(Long id) {
        if (id!=null){
            match_id=id;
        }
    }
