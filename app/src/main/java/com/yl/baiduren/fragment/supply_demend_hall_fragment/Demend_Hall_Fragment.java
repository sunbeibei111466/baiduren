...
public class Demend_Hall_Fragment extends BaseFragment implements Demend_Hall_Adpater.Item_getId {
    ...
    public int loadWindowLayout() {
        return R.layout.demend_hall;
    }

    @Override
    public void initViews(View rootView) {
        refreshLayout = rootView.findViewById(R.id.demend_hall_refresh);
        refreshLayout.setMaterialRefreshListener(refreshListener);
        supply_hall_recycleview = rootView.findViewById(R.id.demend_hall_recycleview);
        LinearLayoutManager manager = new LinearLayoutManager(getActivity());
        manager.setOrientation(LinearLayoutManager.VERTICAL);
        supply_hall_recycleview.setLayoutManager(manager);
        adapter = new Demend_Hall_Adpater(getActivity());
    ...
    private void requestWork() {
        if (UserInfoUtils.IsLogin(getActivity())) {
            ...
            RetrofitHelper.getInstance(getActivity()).getServer()
                    .getHall_Demend(signature, RequestBody.create(MediaType.parse(Constant.JSON), json))
